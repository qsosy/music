...
def creat_file(path, fname):
    new_folder_path = os.path.join(path, fname)
    #new_folder_path = os.path.join(path,"中文名字")
...
def move(path, fname, ky=0):
    creat_file(path, fname)
    #列出文档
    file_list = os.listdir(path)
    #提取出文档名称内的数字，并根据数字决定将问价发往那个文件夹
    for i in file_list:  
        old_file_path = os.path.join(path,i)
        new_file_path = os.path.join(path, fname)
        ...
        if os.path.isdir(old_file_path):
            ...
            if check_contain_jp(i):
                    shutil.move(old_file_path,new_file_path)
            else:
                if check_ky(ky, i):
                    shutil.move(old_file_path,new_file_path)
        elif "ch" in fname:
            if check_contain_ch(i):
                    shutil.move(old_file_path,new_file_path)
        elif "en" in fname:
            shutil.move(old_file_path,new_file_path)
        else:
...
def main():
    ky = ["op", "ed","神田沙也加", "米津玄師", "AKB48", "akb48", "ELISA", "Lia", "LiSA", "小林未郁", 
          ...
          "高达", "釘宮理恵", "SNoW", "thunder", "高橋洋樹", "井口裕香", "橋本潮", "早安少女组", 
          "池田彩", "工藤真由", "千寻", 
          ]
